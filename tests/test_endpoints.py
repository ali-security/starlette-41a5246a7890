...
def test_http_endpoint_route_head_falls_back_to_get(client: TestClient) -> None:
    response = client.head("/")
    assert response.status_code == 200


def test_http_endpoint_does_not_dispatch_non_verb_method(
...
def test_http_endpoint_does_not_dispatch_internal_attribute(
    test_client_factory: TestClientFactory,
) -> None:
    class Endpoint(HTTPEndpoint):
        async def get(self, request: Request) -> PlainTextResponse:
